...
if (!supabaseUrl || !supabaseServiceKey) {
  throw new Error('Missing supabase configuration. Please check VITE_SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY in .env file');
}
...
const allowedOrigins = NODE_ENV === 'production' 
  ? [process.env.FRONTEND_URL || `http://localhost:${PORT}`].filter(Boolean)
...
// Helper function to resolve ping content for contact
...
async function processMailingQueue(mailingId) {
  ...
  while (true) {
    try {
      ...
      try {
        const sendResult = await sendEmailToRecipient(grabbed.id);
        ...
        failedCount++;
        console.error(`[Queue ${mailingId}] Error when sending to recipient ${grabbed.id}:`, sendErr);
      }
...
// Send email to recipient (internal function)
async function sendEmailToRecipient(recipient_id) {
  try {
    ...
    // Save to Sent folder (fire-and-forget)
    (async () => {
      try {
...
// Send email endpoint (for direct calls)
app.post('/api/send-email', async (req, res) => {
  try {
    ...
    const result = await sendEmailToRecipient(recipient_id);
